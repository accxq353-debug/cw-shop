...
import AdminDiscordBot from "@/components/AdminDiscordBot";
...
export default function AdminApp() {
  ...
  const [tab, setTab] = useState<"orders" | "velocity" | "bot" | "products" | "users">("orders");
  ...
  // Maintenance mode state with configurable countdown timer
  const [maintenanceMode, setMaintenanceMode] = useState(false);
  const [maintenanceLoading, setMaintenanceLoading] = useState(false);
  const [showTimerModal, setShowTimerModal] = useState(false);
  const [mHours, setMHours] = useState(1);
  const [mMinutes, setMMinutes] = useState(45);
  ...
  const saveMaintenanceState = async (enabled: boolean, hours = mHours, minutes = mMinutes, seconds = mSeconds) => {
    ...
    try {
      ...
      setSuccessMsg(
        data.maintenance
          ? `Parduotuvės atnaujinimo rėžimas ĮJUNGTAS! Laikmatis nustatytas: ${hours} val. ${minutes} min.`
          : "Parduotuvės atnaujinimo rėžimas IŠJUNGTAS — parduotuvė vėl atidaryta lankytojams."
      );
      setShowTimerModal(false);
    } catch {
      setError("Nepavyko atnaujinti priežiūros rėžimo nustatymų.");
    } finally {
  ...
  return (
    <div>
      ...
      {/* Restocking Timer Config Modal */}
      {showTimerModal ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl border border-warn/50 bg-panel p-6 shadow-2xl">
            <div className="flex items-center justify-between border-b border-line pb-3">
              <div className="font-display text-[18px] font-black text-white flex items-center gap-2">
                <span>⏱️</span>
                Nustatyti Restocking Laikmatį
              </div>
              <button
                type="button"
            ...
            <p className="mt-3 text-[13.5px] leading-relaxed text-muted">
              Prieš įjungiant atnaujinimo rėžimą lankytojams, nurodykite, kiek laiko rodyti atgalinį skaičiavimą (Valandos / Minutės):
            </p>

            <div className="mt-5 grid grid-cols-3 gap-3">
              <div>
                <label className="micro block text-muted/70 mb-1">Valandos</label>
                <input
                  type="number"
                  min="0"
                  max="72"
                  value={mHours}
                  onChange={(e) => setMHours(Math.max(0, parseInt(e.target.value) || 0))}
                  className="w-full rounded-xl border border-line bg-void px-3 py-2.5 font-mono text-[16px] text-center font-bold text-signal outline-none focus:border-signal"
                />
              </div>

              <div>
                <label className="micro block text-muted/70 mb-1">Minutės</label>
                <input
                  type="number"
                  min="0"
                  max="59"
                  value={mMinutes}
                  onChange={(e) => setMMinutes(Math.max(0, Math.min(59, parseInt(e.target.value) || 0)))}
                  className="w-full rounded-xl border border-line bg-void px-3 py-2.5 font-mono text-[16px] text-center font-bold text-signal outline-none focus:border-signal"
                />
              </div>

              <div>
                <label className="micro block text-muted/70 mb-1">Sekundės</label>
                <input
                  type="number"
                  min="0"
                  max="59"
                  value={mSeconds}
                  onChange={(e) => setMSeconds(Math.max(0, Math.min(59, parseInt(e.target.value) || 0)))}
                  className="w-full rounded-xl border border-line bg-void px-3 py-2.5 font-mono text-[16px] text-center font-bold text-signal outline-none focus:border-signal"
                />
              </div>
            </div>
            ...
            <div className="mt-6 flex justify-end gap-2.5 pt-3 border-t border-line">
              ...
              <button
                type="button"
                onClick={() => saveMaintenanceState(true, mHours, mMinutes, mSeconds)}
                className="rounded-xl bg-warn px-5 py-2 font-display text-[14px] font-extrabold text-black hover:opacity-90 shadow-md"
              >
                Įjungti Restocking Rėžimą
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        {[
          ["Visi", (orders ?? []).length || 0, "", "text-white"],
          ["Laukiama", counts["laukiama"] ?? 0, "laukiama", "text-signal"],
          ["Pateikta", counts["mokejimas_pateiktas"] ?? 0, "mokejimas_pateiktas", "text-warn"],
          ["Patvirtinta", counts["patvirtinta"] ?? 0, "patvirtinta", "text-live"],
          ["Įvykdyta", counts["ivykdyta"] ?? 0, "ivykdyta", "text-emerald-400"],
          ["Atmesta", counts["atmesta"] ?? 0, "atmesta", "text-red-400"],
        ].map(([label, value, filterVal, colorClass]) => {
          const isSelected = status === filterVal && tab === "orders";
          return (
            <button
              key={String(label)}
              type="button"
              onClick={() => {
                setTab("orders");
                setStatus(String(filterVal));
              }}
              className={`rounded-2xl border p-4 text-left transition-all duration-200 hover:-translate-y-1 ${
                isSelected
                  ? "border-signal bg-signal/15 shadow-[0_0_20px_rgba(183,139,255,0.25)]"
                  : "border-line bg-panel hover:border-white/20"
              }`}
            >
              <div className="micro text-muted/70">{label}</div>
              <div className={`num mt-2 text-[26px] font-black ${colorClass}`}>
                {value}
              </div>
            </button>
          );
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {(["orders", "velocity", "bot", "products", "users"] as const).map((t) => (
          <button
            key={t}
            type="button"
            onClick={() => setTab(t)}
          ...
          >
            {t === "orders"
              ? "Užsakymai"
              : t === "velocity"
                ? "📊 Sales Velocity & Rizika"
                : t === "bot"
                  ? "👾 Discord Bot & Bilietai"
                  : t === "products"
                    ? "Prekių valdymas & Kainos"
                    : "Vartotojai ir Adminai"}
          </button>
        ))}
        {tab === "orders" && orders ? (
              ...
              return (
                <article key={o.code} className="panel rounded-2xl">
                  <div className="flex w-full flex-wrap items-center gap-x-5 gap-y-3 p-5 text-left">
                    ...
                    >
                      <div>
                        <div className="micro text-muted/70">Užsakymas / Invoice</div>
                        <div className="flex items-center gap-2">
                          <span className="num text-[18px] font-black text-signal">#{o.code}</span>
                          <span
                            onClick={(e) => {
                              e.stopPropagation();
                              navigator.clipboard.writeText(o.code);
                              setSuccessMsg(`Užsakymo kodas #${o.code} nukopijuotas!`);
                            }}
                            title="Kopijuoti sąskaitos kodą"
                            className="cursor-pointer text-muted/60 hover:text-signal text-[12px] p-0.5"
                          >
                            📋
                          </span>
                        </div>
