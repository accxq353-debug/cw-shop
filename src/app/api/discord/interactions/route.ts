...
const LOGO_THUMBNAIL = "https://raw.githubusercontent.com/cw-shop/assets/main/cw-shop-logo.png";
...
export async function POST(req: Request) {
  ...
  if (interaction.type === InteractionType.APPLICATION_COMMAND) {
    ...
    // Exact match for the ticket panel screenshot
          ...
          embeds: [
            {
              ...
              description: [
                ...
                `• Already bought from SellAuth? Claim your role here`,
              ].join("\n"),
  ...
  if (interaction.type === InteractionType.MESSAGE_COMPONENT) {
    ...
    if (customId === "open_ticket_select") {
      ...
      // If Purchase -> Return Modal exactly matching user screenshot
      ...
      const resData = await createHttpTicket(interaction, selected, "General Support", 1);
      return NextResponse.json(resData);
    }

    // Set Amount Button -> Opens modal for staff to set amount in EUR
    if (customId?.startsWith("btn_set_amount_")) {
      const ticketNum = customId.replace("btn_set_amount_", "");
      return NextResponse.json({
        type: InteractionResponseType.MODAL,
        data: {
          custom_id: `modal_set_amount_${ticketNum}`,
          ...
          components: [
            {
              ...
              components: [
                {
                  type: 4,
                  custom_id: "input_amount_eur",
                  label: "Amount in EUR (e.g. 1.00 or 15.50)",
                  style: 1,
                  placeholder: "1.00",
                  required: true,
                },
    ...
    if (customId === "btn_ping_staff") {
      const channelId = interaction.channel_id;
      const staffPings = "@here Staff requested!";
      await discordRest(`/channels/${channelId}/messages`, "POST", {
        content: staffPings,
      });
      ...
      return NextResponse.json({
        ...
        data: {
          flags: InteractionResponseFlags.EPHEMERAL,
          content: "🔔 Staff was notified!",
        },
      });
    }

    // Payment method dropdown selection inside ticket
    if (customId?.startsWith("select_payment_method_")) {
      const selectedMethod = interaction.data?.values?.[0] || "paypal";
      const parts = customId.replace("select_payment_method_", "").split("_");
      const amountStr = parts[0] || "1.00";
      const channelId = interaction.channel_id;
      const username = interaction.member?.user?.username || interaction.user?.username || "Pirkėjas";
      ...
      if (selectedMethod === "bank") {
        ...
        copyText = `IBAN: LT804010051005860181\nName: Aironas Zonys\nDescription: Papildymas\nAmount: ${amountStr}€`;
        ...
        copyText = `LYknAU9LGs23yU6GJymKsfXE5FMhn5itK8`;
      } else {
        paymentEmbedDescription = [
          `**Amount due:** **${amountStr}€**`,
          ``,
        ...
        embeds: [
          {
            ...
            color: 0x8a2be2,
            description: paymentEmbedDescription,
            footer: { text: "Send your proof screenshot in this channel" },
            timestamp: new Date().toISOString(),
          },
        ],
        components: [
      ...
      return NextResponse.json({
        ...
        data: {
          flags: InteractionResponseFlags.EPHEMERAL,
          content: `Payment details sent! You selected **${selectedMethod.toUpperCase()}**.\n\n\`\`\`\n${copyText}\n\`\`\``,
        },
      });
    }
  ...
  if (interaction.type === InteractionType.MODAL_SUBMIT) {
    ...
    if (customId === "modal_ticket_purchase") {
    ...
    // Staff submitted amount to pay
    if (customId?.startsWith("modal_set_amount_")) {
      const rawAmount = interaction.data.components?.[0]?.components?.[0]?.value || "1.00";
      const cleanNum = parseFloat(rawAmount.replace(/[^0-9.]/g, "")) || 1.0;
      const formattedEur = cleanNum.toFixed(2);
      const channelId = interaction.channel_id;
      const staffName = interaction.member?.user?.username || interaction.user?.username || "Staff";

      // Post the exact "Amount To Pay" embed from the screenshot
      await discordRest(`/channels/${channelId}/messages`, "POST", {
        ...
        components: [
          {
            ...
            components: [
              {
                type: 3,
                custom_id: `select_payment_method_${formattedEur}`,
            ...
            components: [
              {
                ...
                style: ButtonStyleTypes.SECONDARY,
                label: "📋 Copy Amount",
                custom_id: `copy_amt_${formattedEur}`,
                disabled: true,
              },
            ],
          },
      ...
      return NextResponse.json({
        ...
        data: {
          flags: InteractionResponseFlags.EPHEMERAL,
          content: `✅ Mokėtina suma nustatyta į **${formattedEur}€** ir paskelbta biliete!`,
        },
      });
    }
  }
...
async function createHttpTicket(interaction: any, category: string, productInfo: string, qty: number) {
  ...
  const descText = isPurchase
    ? [
        `Hello <@${userId}>! Thanks for reaching out to buy something outside of SellAuth.`,
        ``,
        `A staff member will confirm the **total amount** shortly — sit tight.`,
        ``,
        `**Item**                      **Quantity**            **Opened by**`,
        `\`${productInfo.padEnd(20, " ")}\`  \`${String(qty).padEnd(8, " ")}\`  <@${userId}>`,
        ``,
        `**Status**`,
        `⏳ Awaiting amount from staff`,
      ].join("\n")
    : [
        `Hello <@${userId}>, a staff member will be with you shortly.`,
        ``,
        `Please describe your issue including your **order ID** if applicable.`,
        ``,
        `**Opened by**                 **Status**`,
        `<@${userId}>             ⏳ Waiting on staff`,
      ].join("\n");
          ...
          ? [
              {
                ...
                label: "💰 Set Amount",
                custom_id: `btn_set_amount_${ticketNumber}`,
              },
            ]
          : []),
        {
    ...
    embeds: [
      {
        ...
        description: descText,
        thumbnail: {
          url: "https://raw.githubusercontent.com/cw-shop/assets/main/cw-shop-logo.png",
        },
        footer: { text: `Ticket opened by ${username}` },
        timestamp: new Date().toISOString(),
      },
    ],
