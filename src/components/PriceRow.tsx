...
import { priceLabel, type Product } from "@/data/catalog";
    ...
    >
      ...
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
          <h3 className="truncate font-display text-[16px] font-extrabold tracking-[-0.02em] sm:text-[18px] group-hover:text-signal transition-colors">
            {product.name}
          </h3>
          {product.badge ? (
      ...
      <div className="hidden sm:block">
        <div className="micro text-muted/60">{t.unitsLabel}</div>
        <div className="num mt-1 text-[12px] text-muted">
          {units.length ? units.join(" · ") : `${product.variants.length} ${t.variantsInCatalog.split(" ")[0]}`}
        </div>
      </div>

      <div className="text-right">
        <div className="num text-[15px] font-bold text-ink sm:text-[17px] group-hover:text-signal transition-colors">
          {priceLabel(product)}
        </div>
        <div className="micro mt-1 inline-flex items-center gap-1 text-muted transition-colors group-hover:text-signal">
          {t.chooseBtn} <ArrowRight size={12} strokeWidth={2.6} />
        </div>
      </div>
    </Link>
  );
