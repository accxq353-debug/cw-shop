  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[minmax(0,470px)_minmax(0,1fr)]">
      ...
      <div>
        ...
        <p className="mt-4 max-w-[62ch] text-[14.5px] leading-relaxed text-muted/85">
          {product.description}
        </p>
        ...
        <div className="mt-7 flex flex-wrap items-center gap-4">
          ...
          <button
            type="button"
            onClick={onAdd}
            className={`ml-auto inline-flex items-center gap-2.5 rounded-xl px-7 py-4 font-display text-[15px] font-extrabold transition-transform hover:-translate-y-0.5 ${
              added ? "bg-live text-void" : "bg-white text-void"
            }`}
          >
            {added ? (
              <>
        ...
        <ul className="mt-9 space-y-3 border-t border-line pt-6 text-[14px] text-muted">
          <li className="flex items-start gap-3">
            ...
            Pateikus užsakymą ir patvirtinus mokėjimą, prekę išsiunčiame per{" "}
            {product.delivery}.
          </li>
          <li className="flex items-start gap-3">
          ...
          <li className="flex items-start gap-3">
            ...
            Atsiskaitymas PayPal, siunčiant kaip Friends &amp; Family. Užsakymo
            numerį nurodyk mokėjimo laukelyje.
          </li>
        </ul>
      </div>
    </div>
