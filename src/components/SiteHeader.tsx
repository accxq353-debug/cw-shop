...
import { playSound, isSoundEnabled, toggleSound } from "@/lib/audio";
...
export default function SiteHeader() {
  ...
  const navItems = [
    ...
    { href: "/produktai?kat=zaidimai", label: t.games, icon: Gamepad2 },
    { href: "/produktai?kat=prenumeratos", label: t.subs, icon: Clapperboard },
    { href: "/produktai", label: t.catalog, icon: Package },
    { href: "/atsiliepimai", label: t.reviews, icon: MessageCircle },
    { href: "/duk", label: t.faq, icon: Star },
  ];
  ...
  return (
    <>
      <div className="relative z-40 border-b border-line bg-void-2/90">
        <div className="mx-auto flex max-w-[1180px] flex-wrap items-center justify-center gap-x-4 gap-y-1 px-4 py-2 text-center text-[12.5px] text-muted">
          ...
          >
            {t.clickHere} <ArrowUpRight size={13} strokeWidth={2.8} />
          </Link>
        </div>
      </div>
        ...
        <div className="mx-auto flex max-w-[1180px] items-center gap-3 px-4 py-3.5">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button
              ...
              title={soundOn ? "Garso efektai: ĮJUNGTI (Sound FX: ON)" : "Garso efektai: IŠJUNGTI (Sound FX: OFF)"}
              className={`flex h-9 w-9 items-center justify-center rounded-full border transition-all ${
                soundOn
                  ? "border-signal/50 bg-signal/15 text-signal shadow-[0_0_12px_rgba(183,139,255,0.25)]"
            ...
            <div className="flex items-center rounded-full border border-line bg-white/[0.03] p-1 text-[12px] font-bold">
              <button
                type="button"
                onClick={() => {
                  setLang("lt");
