...
async function handleBotInteraction(interaction: Interaction) {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    // Instant response for /ping without deferring
    if (commandName === "ping") {
  ...
  if (interaction.isStringSelectMenu() && interaction.customId === "open_ticket_select") {
    ...
    // For Purchase: show modal immediately (Discord requires modal to be shown in initial interaction turn without deferring)
    if (val === "purchase") {
    ...
    // Direct ticket creation for other categories
...
async function createTicketChannel(
  ...
  // CRITICAL: Defer reply immediately so Discord doesn't throw "The application didn't respond in time"
