...
async function handleBotInteraction(interaction: Interaction) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          [
            ...
            `• Already bought from SellAuth? Claim your customer role or replacement.`,
          ].join("\n")
        )
  ...
  if (interaction.isStringSelectMenu() && interaction.customId === "open_ticket_select") {
    ...
    // For Purchase: show modal immediately
    if (val === "purchase") {
  ...
  // Modal Submit (Set Amount EUR by staff)
  if (interaction.isModalSubmit() && interaction.customId.startsWith("modal_set_amount_")) {
    const rawAmount = interaction.fields.getTextInputValue("input_amount_eur");
    const cleanNum = parseFloat(rawAmount.replace(/[^0-9.]/g, "")) || 1.0;
    const formattedEur = cleanNum.toFixed(2);
    const channel = interaction.channel as TextChannel;
    const staffName = interaction.user.username;

    if (channel && "send" in channel) {
      const amountEmbed = new EmbedBuilder()
        .setTitle(`💶 Amount To Pay`)
        .setColor(0x8a2be2)
        .setDescription(
          [
            `**Total: ${formattedEur}€**`,
            ``,
            `Select a payment method below to see the payment details.`,
          ].join("\n")
        )
        .setFooter({ text: `Amount set by ${staffName}` })
        .setTimestamp();

      const payMenu = new StringSelectMenuBuilder()
        .setCustomId(`select_payment_method_${formattedEur}`)
      ...
      const copyBtn = new ButtonBuilder()
        .setCustomId(`copy_amt_${formattedEur}`)
      ...
      await channel.send({
        content: `@here The amount to pay has been set.`,
        embeds: [amountEmbed],
        components: [
          new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(payMenu),
          new ActionRowBuilder<ButtonBuilder>().addComponents(copyBtn),
        ],
      });
    }

    await interaction.reply({ content: `✅ Mokėtina suma nustatyta į **${formattedEur}€**!`, ephemeral: true });
  }

  // Set Amount Button
  if (interaction.isButton() && interaction.customId.startsWith("btn_set_amount_")) {
    const ticketNum = interaction.customId.replace("btn_set_amount_", "");
    const modal = new ModalBuilder()
      .setCustomId(`modal_set_amount_${ticketNum}`)
      .setTitle("Set Amount To Pay");

    const amountInput = new TextInputBuilder()
      .setCustomId("input_amount_eur")
      .setLabel("Amount in EUR (e.g. 1.00 or 15.50)")
      .setPlaceholder("1.00")
      .setStyle(TextInputStyle.Short)
      .setRequired(true);
  ...
  if (interaction.isButton() && interaction.customId === "btn_ping_staff") {
    const channel = interaction.channel as TextChannel;
    if (channel && "send" in channel) {
      await channel.send({ content: "@here Staff requested by customer!" });
    }
    await interaction.reply({ content: "🔔 Staff has been notified!", ephemeral: true });
  }

  // Payment method select
  if (interaction.isStringSelectMenu() && interaction.customId.startsWith("select_payment_method_")) {
    const selectedMethod = interaction.values[0] || "paypal";
    const parts = interaction.customId.replace("select_payment_method_", "").split("_");
    const amountStr = parts[0] || "1.00";
    const channel = interaction.channel as TextChannel;

    let paymentEmbedDescription = "";

    if (selectedMethod === "bank") {
      paymentEmbedDescription = [
        `**Amount due:** **${amountStr}€**`,
        ``,
        `**Bank Transfer**`,
        `IBAN: \`LT804010051005860181\``,
        `Name: \`Aironas Zonys\``,
        `Description: \`Papildymas\``,
        ``,
        `**After sending payment, upload a screenshot as proof in this channel. Staff will be notified automatically.**`,
      ].join("\n");
    } else if (selectedMethod === "ltc") {
      paymentEmbedDescription = [
        `**Amount due:** **${amountStr}€**`,
        ``,
        `**Litecoin (LTC)**`,
        `Address: \`LYknAU9LGs23yU6GJymKsfXE5FMhn5itK8\``,
      ...
      paymentEmbedDescription = [
        ...
        `Email: \`dendepro11@gmail.com\``,
        `⚠️ **Send as Friends & Family — no notes.**`,
        ``,
        `**After sending payment, upload a screenshot as proof in this channel. Staff will be notified automatically.**`,
      ].join("\n");
    }
    ...
    if (channel && "send" in channel) {
      const embed = new EmbedBuilder()
        ...
        .setColor(0x8a2be2)
        .setDescription(paymentEmbedDescription)
        .setFooter({ text: "Send your proof screenshot in this channel" })
        .setTimestamp();

      const copyBtn = new ButtonBuilder()
        .setCustomId("copy_dummy_btn")
        .setLabel("Copy Payment Info")
        .setStyle(ButtonStyle.Secondary)
        .setEmoji("📋")
        .setDisabled(true);
      ...
      await channel.send({
        embeds: [embed],
        components: [new ActionRowBuilder<ButtonBuilder>().addComponents(copyBtn)],
      });
    }

    await interaction.reply({ content: `Payment method selected: **${selectedMethod.toUpperCase()}** (${amountStr}€)`, ephemeral: true });
  }
  ...
  if (interaction.isButton() && interaction.customId === "btn_close_ticket") {
    const channel = interaction.channel as TextChannel;
    await interaction.reply({ content: "Bilietas uždaromas po 5 sekundžių..." }).catch(() => {});
...
async function createTicketChannel(
  ...
  if (!guild) {
    if (interaction.isRepliable()) {
      await interaction.reply({ content: "Klaida: Bilietas gali būti kuriamas tik serveryje.", ephemeral: true }).catch(() => {});
    }
    return;
  }
  ...
  const cleanUsername = (interaction.user?.username || "user").toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 10);
  ...
  try {
    ...
    const titleText = isPurchase ? "💶 Purchase Ticket" : "❓ Support Ticket";

    const descText = isPurchase
      ? [
          `Hello ${interaction.user.toString()}! Thanks for reaching out to buy something outside of SellAuth.`,
          ``,
          `A staff member will confirm the **total amount** shortly — sit tight.`,
          ``,
          `**Item**                      **Quantity**            **Opened by**`,
          `\`${productInfo.padEnd(20, " ")}\`  \`${String(qty).padEnd(8, " ")}\`  ${interaction.user.toString()}`,
          ``,
          `**Status**`,
          `⏳ Awaiting amount from staff`,
        ].join("\n")
      : [
          `Hello ${interaction.user.toString()}, a staff member will be with you shortly.`,
          ``,
          `Please describe your issue including your **order ID** if applicable.`,
          ``,
          `**Opened by**                 **Status**`,
          `${interaction.user.toString()}             ⏳ Waiting on staff`,
        ].join("\n");
    ...
    const embed = new EmbedBuilder()
      ...
      .setThumbnail("https://raw.githubusercontent.com/cw-shop/assets/main/cw-shop-logo.png")
    ...
    if (isPurchase) {
      buttonsRow.addComponents(
        new ButtonBuilder()
          .setCustomId(`btn_set_amount_${ticketNumber}`)
          .setLabel("Set Amount")
    ...
    await channel.send({ content: `${interaction.user.toString()}`, embeds: [embed], components });

    if (interaction.deferred) {
      await interaction.editReply({ content: `Jūsų bilietas sukurtas: ${channel.toString()}` }).catch(() => {});
    } else {
      await interaction.reply({ content: `Jūsų bilietas sukurtas: ${channel.toString()}`, ephemeral: true }).catch(() => {});
    }
  } catch (err) {
    console.error("Error creating ticket channel:", err);
    if (interaction.deferred) {
      await interaction.editReply({ content: "Nepavyko sukurti bilieto kanalo. Patikrinkite boto teises serveryje (Administrator / Manage Channels)." }).catch(() => {});
    } else {
      await interaction.reply({ content: "Nepavyko sukurti bilieto kanalo. Patikrinkite boto teises serveryje.", ephemeral: true }).catch(() => {});
    }
  }
}
