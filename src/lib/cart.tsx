...
import { productBySlug, products as initialStaticProducts, type Product } from "@/data/catalog";
...
function resolve(lines: CartLine[], dynamicProducts: Product[]): ResolvedLine[] {
  const out: ResolvedLine[] = [];
  for (const line of lines) {
    const product = dynamicProducts.find((p) => p.slug === line.productSlug) || productBySlug(line.productSlug);
    ...
    out.push({
      ...
      allowedMethods: product.allowedMethods || ["paypal", "bank", "ltc"],
    });
  }
  return out;
}
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const [allProducts, setAllProducts] = useState<Product[]>(initialStaticProducts);
  ...
  const value = useMemo<CartState>(() => {
    const resolved = resolve(lines, allProducts);

    // Compute intersection of allowed methods across all items in cart
    let methods: string[] = ["paypal", "bank", "ltc"];
    if (resolved.length > 0) {
      methods = resolved.reduce<string[]>((acc, item, idx) => {
        const itemMethods = item.allowedMethods && item.allowedMethods.length > 0
          ? item.allowedMethods
          : ["paypal", "bank", "ltc"];
        if (idx === 0) return itemMethods;
        return acc.filter((m) => itemMethods.includes(m));
      }, ["paypal", "bank", "ltc"]);
      
      // Fallback if no intersection found
      if (methods.length === 0) {
        methods = resolved[0].allowedMethods || ["paypal", "bank", "ltc"];
      }
    }
  ...
  }, [lines, allProducts, sessionKey, ready, add, setQty, remove, clear]);
